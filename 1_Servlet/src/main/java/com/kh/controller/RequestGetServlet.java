...
//		System.out.println("서버에 요ㅊㅇ이 정상적으로 응ㄷ답");
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		 ...
		 * 두번째 매개변수읜 response는 요청 처리 후 응답에 사용되는 객체
		 * 
		...
		String name = request.getParameter("name"); // "윤대한" | ""
		...
		double height = Double.parseDouble(request.getParameter("city"));
		...
		//2) 응답받는 사용자(요청했던 사람)와의 스트림(클라이언트와의 동로)를 생성
		...
		if(gender == null) {
			...
			if(gender.equals("M")) {
				out.print("<span>남자입니다.</span>");
				
			}else {
				out.print("<span>여자입니다.</span>");
			}
		}
	}
